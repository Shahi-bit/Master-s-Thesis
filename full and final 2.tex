...
Despite substantial advancements in alternative Proton Exchange Membranes, there are still research gap in the literature. For example, most of the research carried out on PVA membranes has focused on water treatment and filtration, with less attention paid to proton exchange for energy conversion.\cite{11} Although there is a great deal of research carried out on phosphoric acid doping of polybenzimidazoles,\cite{20,59} there is only a small amount of research carried out on H\(_3\)PO\(_4\)-doped electrospun PVA nanofiber membranes. Although there is a great deal of research carried out on PVA-citric acid crosslinking, it is mostly focused on dense cast membranes, with less attention paid to electrospun nanofiber membranes,\cite{53,54,68} where the large surface area of the nanofibers may also play a role in proton transport, a factor that is yet to be fully exploited.

The impact of how long you dope the membrane on how much acid it takes up, and how this interplay between proton conductivity and mechanical stability is balanced, is not something that’s been explored. There is a problem with acid retention during aqueous washing procedures, which is a problem that doped polymer electrolytes face, \cite{35} and Wang et al. pointed out how there is a need to better understand the degradation processes that are at work here, quantitatively speaking.\cite{58} Furthermore, there hasn’t been any computational study done using DFT on this particular system, so the molecular-level origin of proton conductivity, including the thermodynamics of acid binding, the actual mechanism of proton transport, and how there is acid retention, hasn’t really been characterized.

The current research attempts to bridge this research gap through a combined approach of hands-on experiments and computer simulations. We electrospun PVA/CA nanofiber membranes and doped them with H\(_3\)PO\(_4\) at three time points: 30 minutes, 2 hours, and 24 hours. The doped nanofiber membranes were characterized using FTIR-ATR spectroscopy, SEM, \textbf{TGA}, XRD, contact angle measurement,, and electrochemical impedance spectroscopy. At the same time, this paper performed 23 sets of density functional theory calculations using the B3LYP-D3BJ/6-31G* method in ORCA 6.1.1 to, for the first time, present a quantum-mechanical description of the binding, proton transfer, and retention properties of H\(_3\)PO\(_4\) in this membrane system.\cite{21,79}
...
The shape of the fibers and the distribution of their diameters were studied using a \textbf{JEOL JSM-7600F}, with support from the NUST MISIS.

\textit{The instrument was operated in the following conditions: voltage - 10 kV, working distance - 10 mm, detector - secondary electron detector in high vacuum mode (\(<10^{-3}\) Pa).}

\textit{For the preparation of the samples, the membrane was placed on a stub using conductive carbon tape and then subjected to a 10 nm thick gold coating using a QUORUM Q150R ES sputter coater to prevent charging during imaging in the SEM.\cite{39}}

\textit{For the measurement of the diameters of the fibers, the images were taken at a magnification of 10,000\(\times\) using ImageJ (NIH, v1.53k). The diameters were measured at least five points in an image, with a minimum of 100 fibers.}

\subsection{Chemical Footprint (FTIR-ATR)}
To verify the composition and crosslinking state, FTIR-ATR spectroscopy was used. The tests were carried out in the\textbf{ analytical laboratory} at NUST MISIS using a spectrometer with a \textit{\textbf{diamond ATR}} attachment. The parameters for collecting the spectra were as follows: scan range – 4000-400 cm\(^{-1}\), resolution – 4 cm\(^{-1}\), number of scans – 64.
...
Static water contact angles were measured using a \textbf{\textit{KRUSS DSA25}} drop shape analyser (KRÜSS GmbH, Germany), located at NUST MISIS. \textit{A water droplet of 5 \(\mu\)L deionised water was placed on the surface of each membrane. The contact angle measurement was performed within 10 seconds after the water droplet touched the membrane surface using the sessile drop method and Young’s equation.\cite{71} At least five measurements were performed at different points on the surface of the membrane samples. The contact angles were then used to calculate the work of adhesion (Wa) and the surface free energy (\(\gamma\)).}
...
We determined the static water contact angles of three membrane samples, namely, the uncrosslinked PVA-CA, the crosslinked PVA-CA (S0), and the H\(_3\)PO\(_4\)-doped membrane (S3), in order to observe the variation of surface wettability during the fabrication process. The contact angle images are shown in Figure 4.3. Using the contact angles, the work of adhesion (Wa) and the surface free energy (\(\gamma\)) were determined by applying Young’s equation.\cite{71}

\subsection{Uncrosslinked PVA-CA (\(\theta = 68.1^\circ\))}
The contact angle of the uncrosslinked membrane is 68.1 degrees, which puts it in the moderately hydrophilic category. The adhesion work of the membrane is 100.0 mJ/m\(^2\), while the surface free energy is 50.0 mJ/m\(^2\). These values are expected, as published values for PVA films indicate that the dense surface hydroxyl groups create a moderate surface energy that is sufficient to attract water molecules but not sufficient to cause them to spread across the surface of the film.\cite{9,29} The 68.1-degree contact angle confirms that the uncrosslinked membrane is chemically active, with the hydroxyl groups fully accessible on the surface of the membrane.

\subsection{Crosslinked PVA-CA (\(\theta = 63.5^\circ\))}
The contact angle is lowered a little by this type of thermal crosslinkeding to 63.5\(^\circ\), but the work of adhesion increases to 105.3 mJ/m\(^2\), and the surface free energy increases to 52.6 mJ/m\(^2\). The chemical implications of this small change are significant because some of the hydroxyl groups on the surface change to ester groups, which are less hydrophilic than free -OH groups. If we were to look at this change in chemical structure by itself, we would expect that the contact angle would increase. The reality is that it decreases, indicating that when we form those ester bonds, we create a smoother, more compact surface topology that makes it easier for the water to spread, but we still have enough free hydroxyls on that surface to increase, or at least sustain, that wettability.\cite{35,53}
This reading is consistent with the FTIR results as seen in Section 4.2.2, where both C=O stretches associated with the ester group and remaining O-H stretches are seen in the crosslinked membrane. This is a confirmation that not all hydroxyl groups are utilized during crosslinking at 10 wt\% citric acid.

\subsection{H\(_3\)PO\(_4\)-Doped Membrane (\(\theta = 31.9^\circ\))}
The most significant change in this series is seen after doping with phosphoric acid. The contact angle is reduced significantly to 31.9\(^\circ\). The reduction is 36.2\(^\circ\) from the baseline of the uncrosslinked membranes and 31.6\(^\circ\) from that of the crosslinked membrane. Since \(\theta\) is 31.9\(^\circ\), it is clear that this membrane is strongly hydrophilic with a work of adhesion equal to 134.6 mJ/m\(^2\) and a surface free energy close to 67.3 mJ/m\(^2\).
This is due to its molecular origins. The addition of H\(_3\)PO\(_4\) includes highly hygroscopic phosphate groups that strongly attract water molecules through hydrogen bonding as well as the cooperative binding network as calculated by DFT, where multiple H\(_3\)PO\(_4\) molecules simultaneously bind to the neighboring PVA hydroxyl groups with a total binding energy of 203.0 kJ/mol. This is a densely populated surface with high affinity for water molecules, which strongly attract sessile drops with extremely low contact angles.\cite{21,55} As the drop shape is flat with the contact angle at approximately 32\(^\circ\), the macroscopic-level takeaway from this is that this surface thermodynamics is actually consistent with the Grotthuss proton relay network at the molecular level. The same hydrogen bonding network that enables the rapid hopping of protons within the membrane also enables this surface to strongly attract water molecules. This is just one of the consistencies that the combined work of this dissertation highlights.\cite{21,55,71}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.32\linewidth}
        ...
        \caption{PVA/CA (Uncrosslinked) $\theta \approx 68.1^\circ$}
    ...
    \begin{subfigure}{0.32\linewidth}
        ...
        \caption{Crosslinked PVA/CA $\theta \approx 63.5^\circ$}
    ...
    \begin{subfigure}{0.32\linewidth}
        ...
        \caption{H$_3$PO$_4$-Doped (30\%) $\theta \approx 31.9^\circ$}
        \label{fig:ca_doped}
    \end{subfigure}
    \caption{Wettability transition of the membrane series. The contact angle decreases from (a) the uncrosslinked state to (c) the acid-doped state, indicating a significant increase in surface hydrophilicity.}
