...
19.5~kJ\,mol\(^{-1}\) consistent with the range reported for phosphoric‑acid‑doped membranes, and that replacing the hydrogen bond with a covalent 
...
conductivity of two membrane conditions (immersed‑only and immersed + 
heat‑treated) as well as an undoped crosslinked PVA baseline. All 
measurements were performed at $24^{\circ}\text{C}$ and $30\%$ relative 
humidity. The experimental configuration, sample preparation and data 
processing are described in Section~\ref{sec:eis}; the complete set of 
fitted equivalent circuit parameters and the corresponding Nyquist and 
Bode plots are provided in Appendix~\ref{app:eis}.

\subsection{Bulk Resistance and Equivalent Circuit Fitting}

The impedance spectra were fitted to an R1~+~CPE1 equivalent circuit 
using ZView~4. For the heat‑treated membrane the fit gave 
$R_{b}=0.277\ \Omega$ ($\chi^{2}=0.0047$); for the immersed‑only 
membrane $R_{b}=1.45\ \Omega$ ($\chi^{2}=0.0065$). Both values agree within the fitting error with the bulk resistances 
determined by manual interpolation of the high‑frequency Nyquist intercept 
($0.276\ \Omega$ and $1.42\ \Omega$, respectively). The CPE exponent 
$\alpha$ was $0.94$ for the heat‑treated sample and $0.89$ for the 
immersed‑only sample, consistent with a predominantly capacitive 
blocking electrode.
...
The proton conductivity was calculated from the fitted bulk resistance 
using $\sigma = L/(R_{b}A)$, with $L$ the membrane thickness and 
$A=3.14\ \text{cm}^{2}$ the electrode contact area. The results are 
listed in Table~\ref{tab:conductivity-results}. 
...
The undoped membrane is essentially insulating ($\sigma<0.003\ \text{mS\,cm}^{-1}$). 
Immersion in phosphoric acid raises the conductivity to $7.0\ \text{mS\,cm}^{-1}$, 
and a subsequent heat treatment produces a further six‑fold increase to 
$41.4\ \text{mS\,cm}^{-1}$. This dramatic enhancement, obtained at comparable 
acid loading, is attributed to the heat‑induced redistribution of phosphoric acid 
into cooperative binding configurations (Section:DFT-Cooperative) and 
the formation of continuous proton pathways as the fibrous network coalesces 
(Section:SEM).

\subsection{Comparison with Nafion and Literature}

Table~\ref{tab:conductivity-comparison} places the present results in context. 
Nafion~117 reaches $90$--$100\ \text{mS\,cm}^{-1}$ under full hydration but 
loses much of its conductivity at low humidity. The heat‑treated PVA/CA/H$_{3}$PO$_{4}$ 
membrane ($41.4\ \text{mS\,cm}^{-1}$, $30\%$ RH) therefore offers a competitive 
alternative for low‑humidity operation.
...
Variable‑temperature EIS was not performed within the present study; therefore 
Arrhenius activation energies are not reported. The room‑temperature conductivity 
data remain sufficient for comparison with the DFT proton‑transfer barriers 
calculated in Chapter~5.

\subsection{Non‑Linear Conductivity Enhancement: Molecular Interpretation}

The factor‑of‑six conductivity difference between the immersed‑only and 
heat‑treated membranes, despite comparable acid loading, demonstrates that 
proton conduction is governed by the binding configuration of the acid rather 
than by its total amount. As discussed in section DFT-Cooperative, 
heat treatment allows the acid to relocate from isolated single‑site binding 
positions (binding energy $-78.1\ \text{kJ\,mol}^{-1}$) into cooperative 
two‑molecule assemblies (combined binding energy $-203.0\ \text{kJ\,mol}^{-1}$), 
creating shorter and more efficient proton‑relay contacts that give rise to the 
observed superlinear conductivity gain. This direct link between cooperative 
binding and macroscopic transport is one of the principal original contributions 
...
The experimental conductivity measurements revealed a factor‑of‑six enhancement between the immersed‑only membrane (7.2~mS\,cm\textsuperscript{\(-\)1}) and the heat‑treated membrane (41.5~mS\,cm\textsuperscript{\(-\)1}) at a comparable total acid loading (Section~4.4.3). 
This superlinear gain cannot be explained by an increase in acid quantity alone.
The DFT binding energies provide the molecular explanation: the first phosphoric acid molecule binds to an isolated hydroxyl site at \( -78.1~\mathrm{kJ\,mol^{-1}} \). 
When a second molecule binds cooperatively at a neighbouring site, the combined binding energy reaches \( -203.0~\mathrm{kJ\,mol^{-1}} \), corresponding to \( -101.5~\mathrm{kJ\,mol^{-1}} \) per molecule — a 30\% enhancement relative to the isolated case. 
These cooperatively bound molecules participate in shorter, stronger hydrogen bonds that lower the effective proton transfer barriers and create more efficient relay chains, producing a conductivity that increases much faster than the acid loading.
Thus the non‑linear relationship between acid content and conductivity arises from a cooperative improvement in binding quality, not simply from adding more charge carriers \cite{21,55,100}.
...
Figure~\ref{fig:comsol-potential} presents the electrolyte potential distribution 
and current density streamlines across the membrane domain at a cell voltage of 
0.6~V. The surface colour map spans from approximately $-0.50$~V (blue, cathode 
side) to $-0.10$~V (red, anode side), indicating a smooth, monotonic potential 
drop across the $360\,\mu\text{m}$ membrane thickness. The streamlines are strictly 
vertical and evenly spaced over the full $100\,\mu\text{m}$ lateral width, with no 
lateral deviation or current crowding.

This uniform flow field confirms that proton transport through the membrane is 
ohmic‑dominated and spatially homogeneous. Any heterogeneity in acid distribution, 
local swelling, or incomplete fibre fusion would produce lateral current gradients; 
their complete absence supports the picture of a well‑developed, continuous proton 
conducting network that the DFT and MD simulations describe at the molecular scale 
\cite{66,111}.
...
The polarisation curve obtained from the model is shown in Figure~\ref{fig:comsol-iv} 
and the corresponding numerical values are listed in Table~\ref{tab:comsol-iv-exp}. 
The total cell current increases monotonically from 0.75~A at $V_\text{cell}=0.2$~V 
to 3.60~A at $V_\text{cell}=1.0$~V. Over the entire voltage range the current--voltage 
relation is perfectly linear ($R^2 > 0.999$), characteristic of a pure ohmic conductor.

\begin{table}[H]
\centering
\caption{COMSOL‑computed cell voltage and total current for the 
PVA/CA/H$_3$PO$_4$ membrane model at 353.15~K. Membrane conductivity input: 
$\sigma_m = 41.4\ \mathrm{mS\,cm^{-1}}$ (experimental value, Section~4.4). 
Membrane thickness 360~$\mu$m, electrode area 3.14~cm$^2$.}
\label{tab:comsol-iv-exp}
\begin{tabular}{cc}
\toprule
Cell Voltage, $V_\text{cell}$ (V) & Total Current (A) \\
\midrule
0.20 & 0.75 \\
0.30 & 1.45 \\
0.40 & 2.15 \\
...
\caption{Polarisation curve for the heat‑treated PVA/CA/H$_3$PO$_4$ membrane 
at 353.15~K, computed using the experimentally measured proton conductivity 
(41.4~mS\,cm$^{-1}$). The strictly linear response confirms ohmic‑dominated 
transport.}
\label{fig:comsol-iv}
\end{figure}

The effective membrane resistance extracted from the slope of the polarisation 
curve is $R = \Delta V / \Delta I = 1.0\ \mathrm{V} / 3.60\ \mathrm{A} 
\approx 0.278\ \Omega$. This value is in excellent agreement with the bulk 
resistance $R_b = 0.276\ \Omega$ obtained directly from the experimental Nyquist 
plot (Section~4.4). The near‑perfect match ($< 1\%$ deviation) confirms that 
the independently measured EIS conductivity and membrane thickness are internally 
consistent and propagate correctly through the continuum model. It also 
demonstrates that the device‑scale performance predicted by COMSOL is a faithful 
one‑dimensional ohmic response, exactly as expected for a homogeneous proton 
conductor at 353~K where electrode activation overpotentials are negligible 
\cite{108,111}.

\section{Integrated Multiscale Correlation}

The three computational tiers of this dissertation converge on a self‑consistent 
picture of the PVA/CA/H\textsubscript{3}PO\textsubscript{4} membrane. DFT establishes 
the energetic foundation: the P--O--C covalent bond has a desorption barrier of 
$251.9\ \mathrm{kJ\,mol^{-1}}$, making acid loss thermodynamically prohibitive, and 
the proton transfer barrier of $38.2\ \mathrm{kJ\,mol^{-1}}$ in the hydrated crosslinked 
model is consistent with the Grotthuss hopping mechanism. MD validates these predictions 
dynamically: the P‑atom MSD remains below $1\ \text{\AA}^2$ over $500$ ps at $353$ K, 
confirming covalent immobilisation, while the persistent RDF peak at $4.7$--$4.8$~\AA\ 
confirms structural stability of the P--O--C linkage at operating temperature.

COMSOL translates the resulting transport properties to the device scale. When supplied 
with the experimentally measured proton conductivity of the heat‑treated membrane 
($41.4\ \mathrm{mS\,cm^{-1}}$, Section~4.4), the model yields a strictly linear 
polarisation curve and a uniform current distribution. The effective membrane 
resistance extracted from the COMSOL polarisation curve ($0.278\ \Omega$) matches 
the bulk resistance measured directly by EIS ($0.276\ \Omega$) to within $0.7\%$. 
This near‑perfect agreement confirms that the molecular‑level insights obtained from 
DFT and MD -- cooperative acid binding, Grotthuss‑dominated proton hopping, and 
immobilisation of acid through covalent anchoring -- propagate correctly through 
the continuum model and manifest as the homogeneous, ohmic response observed 
experimentally.

Each tier feeds the next: bonding energy from DFT is preserved in MD structural 
stability, which underpins the measured conductivity that drives the COMSOL device 
performance. This integrated framework demonstrates that multiscale modelling can 
bridge the gap between quantum chemistry and fuel‑cell‑relevant engineering 
metrics for this class of alternative membrane materials \cite{64}.
