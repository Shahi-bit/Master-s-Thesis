...
Electrochemical impedance spectroscopy (EIS) was used to measure the proton conductivity
of two membrane conditions: PVA membranes immersed in 30 wt\% H\textsubscript{3}PO\textsubscript{4}
without further treatment, and PVA membranes that were heat‑treated at 150\textdegree C for 60 min
after immersion in phosphoric acid. A pure crosslinked PVA membrane without acid doping
was also measured as a baseline. All measurements were performed at 24\textdegree C and 
30\% relative humidity. The results provide the experimental benchmark against which the 
DFT proton transfer barriers computed in Chapter~5 are compared.

\subsection{EIS Measurements and Determination of Bulk Resistance}

Impedance spectra were recorded over the frequency range 100 kHz to 10 mHz using a 
Biologic potentiostat controlled by EC‑Lab software. The raw impedance data (frequency, 
$Z'$, $Z''$) were exported and analysed using a combination of direct table interpolation 
and equivalent circuit fitting in ZView 4. Prior to analysis, the forward frequency sweep 
(100~kHz $\rightarrow$ 10~mHz) was isolated from any reverse sweep to avoid artefacts 
from hysteresis.

For the heat‑treated membrane, the Nyquist curve ($-Z''$ vs.\ $Z'$) crossed the real 
axis within the measured frequency range. The bulk membrane resistance $R_b$ was 
therefore obtained from the high‑frequency intercept by linear interpolation of the two 
data points immediately adjacent to the crossing ($-Z'' = 0$). For the H\textsubscript{3}PO\textsubscript{4}‑only 
membrane, the intercept fell outside the experimental window (all $-Z'' > 0$), and $R_b$ 
was obtained by linear extrapolation of the three highest‑frequency data points 
(100--50~kHz) to zero imaginary impedance.

Both $R_b$ values were independently validated by fitting the full impedance spectrum to an 
equivalent circuit model comprising a series resistor and a constant‑phase element (R1~+~CPE1), 
using ZView~4 software. This model represents the bulk membrane resistance in series with the 
non‑ideal capacitive response of the blocking electrode interface. The fitted R1 values agreed 
with the manually obtained $R_b$ within the fitting error for both samples. The quality of fit 
was assessed by the chi‑squared value ($\chi^2 < 10^{-3}$ for both samples) and by visual 
inspection of the superimposed experimental and fitted Nyquist curves. The CPE exponent 
$\alpha$ was found to be in the range 0.85--0.92, consistent with a predominantly capacitive 
blocking electrode.

For the pure PVA sample, the impedance exceeded $10^3\ \Omega$ with no well‑defined 
intercept, confirming an insulating membrane.

The membrane thickness $L$ was measured by digital micrometer at five positions across 
each sample and the average value used in the conductivity calculation. The electrode 
contact area $A$ was defined by a circular geometry of diameter 2~cm ($A = 3.14\ \text{cm}^2$). 
The proton conductivity $\sigma$ was calculated from the bulk resistance using:
...
Nyquist plots for the two conductive membranes at 24\textdegree C. 
Frequency range 100~kHz--10~mHz. 
Open symbols: experimental data (forward sweep only). 
Solid lines: R1~+~CPE1 equivalent circuit fits. 
Inset: zoom of the high‑frequency region showing determination of $R_b$. 
Arrows mark the bulk resistance intercepts: $R_b = 0.28\ \Omega$ (heat‑treated) 
and $R_b = 1.42\ \Omega$ (H\textsubscript{3}PO\textsubscript{4}‑only).}}
\caption{Nyquist plots and equivalent circuit fits for the two conductive PVA/H\textsubscript{3}PO\textsubscript{4} 
membrane conditions. The high‑frequency intercept on the real axis yields the bulk 
membrane resistance $R_b$.}
\label{fig:nyquist}
\end{figure}
...
\begin{table}[h!]
\centering
\caption{EIS fitted parameters and calculated proton conductivity values at 24\textdegree C, 
30\% RH.}
\label{tab:conductivity-results}
\begin{tabular}{l c c c c}
\hline
Sample & $R_b$ ($\Omega$) & $L$ ($\mu$m) & $\sigma$ (S~cm$^{-1}$) & $\sigma$ (mS~cm$^{-1}$) \\
\hline
Pure PVA (crosslinked, undoped) & ${>}10^3$ & 100 & ${<}3.2 \times 10^{-6}$ & ${<}0.003$ \\
PVA + H\textsubscript{3}PO\textsubscript{4} (immersed) & 1.42 & 320 & $7.2 \times 10^{-3}$ & 7.2 \\
PVA + H\textsubscript{3}PO\textsubscript{4} (immersed + heat‑treated) & 0.276 & 360 & $4.15 \times 10^{-2}$ & 41.5 \\
\hline
\end{tabular}
\end{table}

The measured conductivity values are presented in Table~\ref{tab:conductivity-results}. 
The pure crosslinked PVA membrane without acid doping is essentially insulating, with a 
conductivity below 0.003~mS~cm$^{-1}$. Immersion in 30~wt\% H\textsubscript{3}PO\textsubscript{4} raises 
the conductivity to 7.2~mS~cm$^{-1}$, confirming that phosphoric acid incorporation is the 
primary source of mobile protons in this system.

The most significant result is the approximately six‑fold enhancement achieved by 
heat treatment after acid immersion, producing a conductivity of 41.5~mS~cm$^{-1}$. 
This is consistent with the morphological evolution observed by SEM (Section~4.1): 
heat treatment drives the nanofibrous membrane toward a denser, more homogeneous 
film, eliminating high‑resistance fibre‑to‑fibre junctions and creating continuous 
proton transport pathways. The DFT cooperative binding analysis (Section~5.1.4) provides 
a complementary molecular explanation: the thermal energy supplied during heat treatment 
allows phosphoric acid molecules to redistribute from isolated single‑site binding positions 
into cooperative multi‑point configurations with significantly higher binding energies 
($-156$ to $-203$~kJ~mol$^{-1}$) and shorter, more efficient proton relay contacts.

\subsection{Comparison with Nafion 117 and Published Literature Values}

The conductivity of the heat‑treated membrane (41.5~mS~cm$^{-1}$ at 24\textdegree C, 
30\% RH) is compared with published values in Table~\ref{tab:conductivity-comparison}. 
Nafion~117 under fully humidified conditions at 25\textdegree C delivers approximately 
90--100~mS~cm$^{-1}$.\cite{5,6} However, at the low relative humidity of 30\%, Nafion 
conductivity is known to drop sharply, typically to 1--10~mS~cm$^{-1}$.\cite{3,4,5} 
The heat‑treated PVA/CA/H\textsubscript{3}PO\textsubscript{4} membrane therefore likely 
outperforms Nafion under the dry conditions of the present measurement, an important 
practical advantage for fuel cell operation without external humidification.

Among published PVA‑based membranes, the conductivity of 41.5~mS~cm$^{-1}$ is 
competitive with values reported for PVA/H\textsubscript{3}PO\textsubscript{4} blends 
(10--40~mS~cm$^{-1}$) and exceeds that of PVA/sulfosuccinic acid membranes 
(20--30~mS~cm$^{-1}$).\cite{21,57,58,113}
...
\hline
Material & Doping / Treatment & $\sigma$ (mS~cm$^{-1}$) & T (\textdegree C) & Reference \\
\hline
Nafion 117 & Fully hydrated & 90--100 & 25 & \cite{5,6} \\
Nafion 117 & 30\% RH & $\sim$1--10 & 25 & \cite{3,4,5} \\
PVA/H\textsubscript{3}PO\textsubscript{4} blend & Various & 10--40 & 25 & \cite{57,58} \\
PVA/sulfosuccinic acid & Crosslinked & 20--30 & 25 & \cite{113} \\
\hline
This work — pure PVA & Undoped, crosslinked & ${<}0.003$ & 24 & --- \\
This work — immersed & H\textsubscript{3}PO\textsubscript{4}, no HT & 7.2 & 24 & --- \\
This work — immersed + HT & H\textsubscript{3}PO\textsubscript{4} + 150\textdegree C & 41.5 & 24 & --- \\
\hline
\end{tabular}
\end{table}
...
Temperature‑dependent EIS measurements and Arrhenius activation energy analysis 
were planned but could not be completed within the timeframe of this dissertation. 
The room‑temperature conductivity results reported here are sufficient for comparison 
with the DFT proton transfer barriers calculated in Chapter~5, and the COMSOL device 
model in Section~5.6 uses a literature conductivity value as a placeholder pending the 
completion of these experiments. The experimental setup for variable‑temperature 
impedance measurements is fully prepared, and this work will be prioritised as part of 
ongoing studies.

\subsection{Non‑Linear Conductivity Enhancement: Molecular Interpretation}

The factor‑of‑six difference in conductivity between the immersed‑only and heat‑treated 
membranes — achieved with the same nominal phosphoric acid loading — demonstrates that 
acid quantity alone does not determine performance. The DFT cooperative binding analysis 
in Section~5.1.4 provides the molecular explanation: heat treatment enables a 
thermodynamically driven redistribution of acid molecules from isolated single‑site 
binding positions (binding energy $-78.1$~kJ~mol$^{-1}$) into cooperative two‑molecule 
assemblies with a combined binding energy of $-203.0$~kJ~mol$^{-1}$, equivalent to a 
23.6~kJ~mol$^{-1}$ per‑molecule enhancement. These cooperative binding sites form 
the most efficient proton relay chains with the shortest hydrogen‑bond contacts 
(Table~5.5) and the lowest effective proton transfer barriers, producing the 
superlinear conductivity gain observed experimentally. This direct connection between 
a quantum‑mechanical cooperative binding effect and a macroscopic transport measurement 
constitutes one of the principal original contributions of this dissertation.
...
\section{EIS Data and Equivalent Circuit Parameters}

This appendix presents the electrochemical impedance spectroscopy (EIS) data, 
equivalent circuit fitting results, and calculated proton conductivity values for 
the three membrane conditions studied:

\begin{itemize}
    \item \textbf{Pure PVA} – crosslinked PVA/CA membrane without acid doping.
    \item \textbf{PVA + H\textsubscript{3}PO\textsubscript{4} (immersed)} – membrane 
          immersed in 30~wt\% H\textsubscript{3}PO\textsubscript{4} for 24~h at 
          25\textdegree C, without subsequent heat treatment.
    \item \textbf{PVA + H\textsubscript{3}PO\textsubscript{4} (immersed + heat‑treated)} 
          – membrane immersed in 30~wt\% H\textsubscript{3}PO\textsubscript{4} for 
          24~h and then heat‑treated at 150\textdegree C for 60~min.
\end{itemize}

All measurements were performed at \textbf{24\textdegree C} and \textbf{30\% relative 
humidity} using a Biologic potentiostat controlled by EC‑Lab software. The impedance 
spectra were recorded over the frequency range \textbf{100~kHz to 10~mHz} with an AC 
amplitude of 10~mV (rms). The forward frequency sweep (100~kHz $\rightarrow$ 10~mHz) 
was isolated from any reverse sweep for analysis.

\subsection{Equivalent Circuit Model}
\label{sec:D-equivalent-circuit}

The Nyquist plots ($-Z''$ vs.\ $Z'$) for the two conductive membranes showed a 
characteristic blocking‑electrode response: a linear high‑frequency region 
corresponding to the bulk membrane resistance, followed by a nearly vertical 
low‑frequency spike representing the capacitive electrode–electrolyte interface. 
No semicircle associated with charge‑transfer resistance was observed, confirming 
that the electrodes are blocking for protons.

Accordingly, the spectra were fitted to a simple series \textbf{R1~+~CPE1} 
equivalent circuit using ZView~4 software (Scribner Associates, Inc.). This model 
consists of:

\begin{itemize}
    \item \textbf{R1} – bulk membrane resistance, $R_b$ ($\Omega$).
    \item \textbf{CPE1} – constant‑phase element representing the non‑ideal 
          capacitive behaviour of the blocking electrode interface, characterised 
          by a pseudo‑capacitance $Q$ (F~s$^{\alpha-1}$) and an exponent $\alpha$ 
          ($0 < \alpha \le 1$; $\alpha = 1$ corresponds to an ideal capacitor).
\end{itemize}

The fitted \textbf{R1} value is taken as the bulk membrane resistance $R_b$. 
All three parameters (R1, CPE1‑T, CPE1‑P) were allowed to vary freely during the 
fit. Convergence was achieved with $\chi^2 < 10^{-3}$ for both conductive samples, 
indicating an excellent quality of fit.

\subsection{EIS Results Table}
\label{sec:D-results-table}

The fitted parameters and calculated conductivity values are summarised in 
Table~\ref{tab:D-eis-results}. The proton conductivity $\sigma$ (S~cm$^{-1}$) was 
calculated from the bulk resistance $R_b$ using:

\begin{equation}
\sigma = \frac{L}{R_b \cdot A}
\end{equation}

where $L$ is the membrane thickness (cm) and $A$ is the electrode contact area 
(\SI{3.14}{cm^2}, corresponding to a circular electrode of diameter \SI{2}{cm}). 
Membrane thickness was measured by digital micrometer at five independent positions 
across each sample; the average value is reported.

\begin{table}[h!]
\centering
\caption{EIS fitted parameters and proton conductivity values for the three membrane 
conditions at \SI{24}{\degree C}, 30\% RH.}
\label{tab:D-eis-results}
\begin{tabular}{l c c c c c c}
\hline
Sample & $R_b$ ($\Omega$) & $L$ (\si{\micro m}) & $Q$ (F~s$^{\alpha-1}$) & $\alpha$ & $\sigma$ (S~cm$^{-1}$) & $\sigma$ (mS~cm$^{-1}$) \\
\hline
Pure PVA (undoped) & ${>}10^{3}$ & 100 & --- & --- & ${<}3.2\times10^{-6}$ & ${<}0.003$ \\
PVA + H\textsubscript{3}PO\textsubscript{4} (immersed) & 1.42 & 320 & $1.2\times10^{-4}$ & 0.89 & $7.2\times10^{-3}$ & 7.2 \\
PVA + H\textsubscript{3}PO\textsubscript{4} (immersed + heat‑treated) & 0.276 & 360 & $2.8\times10^{-4}$ & 0.91 & $4.15\times10^{-2}$ & 41.5 \\
\hline
\end{tabular}

\vspace{4pt}
\footnotesize{Note: $R_b$ for the pure PVA sample is a lower‑bound estimate; the 
impedance exceeded \SI{e3}{\Omega} with no discernible real‑axis intercept, 
preventing a reliable fit. CPE parameters ($Q$, $\alpha$) for the conductive samples 
are representative values from the ZView fit; for the pure PVA sample, fitting 
was not performed.}
\end{table}
...
Figure~\ref{fig:D-nyquist} presents the Nyquist plots for the two conductive 
membranes with the fitted R1~+~CPE1 curves superimposed. The high‑frequency 
intercept on the real axis is marked with an arrow and the corresponding $R_b$ 
value. The insets show a magnified view of the high‑frequency region to clarify 
the determination of the bulk resistance.

For the heat‑treated membrane, the curve crosses the real axis within the measured 
frequency range; $R_b$ was obtained by linear interpolation of the two data points 
immediately adjacent to the crossing ($-Z'' = 0$). For the immersed‑only membrane, 
the intercept lies outside the experimental frequency window and was obtained by 
linear extrapolation of the three highest‑frequency data points (100--50~kHz). Both 
manually determined values are in full agreement with the fitted R1 values from the 
ZView analysis.

\begin{figure}[h!]
\centering
\fbox{\parbox{0.9\textwidth}{\centering FIGURE D.1 -- INSERT HERE \\
Nyquist plots for the two conductive PVA/H\textsubscript{3}PO\textsubscript{4} 
membranes at \SI{24}{\degree C}, 30\% RH. \\
Open symbols: experimental data (forward sweep only). \\
Solid lines: R1~+~CPE1 equivalent circuit fits. \\
Insets: magnified high‑frequency region showing the intercept with the real axis.}}
\caption{Nyquist plots and equivalent circuit fits for the immersed‑only and 
immersed~+~heat‑treated PVA/H\textsubscript{3}PO\textsubscript{4} membranes.}
\label{fig:D-nyquist}
\end{figure}

The pure PVA sample (not shown) exhibited an impedance too large to be fitted with 
a simple R1~+~CPE1 model, confirming the insulating nature of the undoped membrane. 
Its Nyquist plot is available on request but was excluded from this appendix due to 
the vastly different impedance scale (tens of thousands of ohms), which precluded 
meaningful visualisation alongside the conductive samples.
