...
a single membrane domain of thickness 360~\(\mu\)m and width 
...
$\sigma_{m} = 41.4\ \mathrm{mS\,cm^{-1}}$, the experimentally 
measured value for the heat‑treated membrane at $24^{\circ}\text{C}$ 
and $30\%$ RH (Section~\ref{sec:conductivity}, 
Table~\ref{tab:conductivity-results}). The membrane thickness was 
$L = 360\ \mu\text{m}$, matching the sample used in the EIS 
measurement. With these experimentally determined inputs, the 
COMSOL model yields a strictly linear polarisation curve and a 
calculated membrane resistance of $0.278\ \Omega$, which agrees 
to within $0.7\%$ with the bulk resistance $R_b = 0.276\ \Omega$ 
obtained from the Nyquist intercept (Section~\ref{sec:conductivity}). 
This agreement confirms the internal consistency of the multiscale 
framework and eliminates the need for a literature placeholder. 
All other boundary conditions and solver settings are documented 
in Appendix F.
...
Immersion of the crosslinked PVA/CA membrane in 30~wt\% H\textsubscript{3}PO\textsubscript{4} without further treatment yields a conductivity of 7.2~mS\,cm\textsuperscript{\(-\)1}, while an identical membrane subjected to heat treatment at 150\textdegree C after immersion reaches 41.5~mS\,cm\textsuperscript{\(-\)1} (Section~4.4.3). 
Despite a comparable total acid loading, the heat‑treated membrane achieves a factor‑of‑six higher conductivity. 
The DFT cooperative binding analysis explains this result: thermal energy during heat treatment allows phosphoric acid molecules to redistribute from isolated single‑site binding positions (binding energy $-$78.1~kJ\,mol\textsuperscript{\(-\)1}$) into cooperative two‑molecule assemblies (combined binding energy $-$203.0~kJ\,mol\textsuperscript{\(-\)1}$). 
...
The same cooperative mechanism is consistent with the non‑linear conductivity progression with doping time reported in previous experimental studies of PVA/H\textsubscript{3}PO\textsubscript{4} membranes \cite{57,58}.
...
In this dissertation we sought answers to 3 questions that only experimental characterisation was unable to resolve, why binds to PVA/CA, how protons travel through the PVA/CA and why only approximately a third of incorporated acid remains after water wash. Thirty one DFT calculations at B3LYP-D3BJ/6-31G* using ORCA~6.1.1 alongside classical molecular dynamics and finite element modeling allowed answers to be obtained for all three.
...
The COMSOL finite element model for the system (with literature conductivity, $\sigma=35~mS\,cm^{-1}$, to be verified by actual EIS experiment) yielded a purely linear polarization curve ($R^2 > 0.999$) and smooth and regular current density streamlines, indicating ohmic-dominated transport in a uniform membrane, and serves as the device-scale closure of the multiscale model.
...
\begin{table}[h]
\centering
\caption{Global parameters of the COMSOL PEMFC model (updated with 
experimental data).}
...
with exchange current density \(i_0 = 1 \times 10^{-3}\)~A\,m\(^{-2}\), 
...
\approx 0.278\ \Omega .
\]
This value agrees with the bulk resistance \(R_b = 0.276\ \Omega\) 
obtained from the Nyquist intercept (Section~\ref{sec:conductivity}) 
to within \(0.7\%\). The near‑perfect match confirms that the 
independently measured EIS conductivity and membrane thickness 
are internally consistent and propagate correctly through the 
continuum model, thereby closing the multiscale validation loop. 
The model is now fully parameterised with experimental data 
and no longer relies on literature estimates.
