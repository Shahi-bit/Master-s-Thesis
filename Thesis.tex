...
\begin{abstract}
 Citric-acid crosslinked, phosphoric acid (H$_3$PO$_4$) doped poly(vinyl alcohol) membranes offer inexpensive fluorine-free membranes for proton conducting applications but they can suffer from severe leaching of H$_3$PO$_4$ during hydration that leads to poor long-term performance. Such a limitation is considered here, by conducting an integrative multiscale experimental and computational study. Experimental nanofiber membranes were produced by following needle-based electrospinning and characterized using scanning electron microscopy (SEM), contact angle measurement, Fourier-transform infrared spectroscopy (FTIR-ATR), and Electrochemical Impedance Spectroscopy(EIS). Material characterization was performed which indicated that $\sim$two-thirds of the irreversible acid originally incorporated using an aqueous medium leaches out of the membrane on exposure to water. To overcome this drawback, we have developed a protocol for thermal-dimension activation after immersion.
 This treatment was also corroborated using electrochemical impedance spectroscopy, which resulted in a six-fold increase in conductivity: from a submerged-only control of \qty{7.0}{mS.cm^{-1}} to a heat-treated membrane of \qty{41.4}{mS.cm^{-1}}. Density functional theory (DFT) cluster models provided a thermodynamic basis for the macroscopic conductivity increase, showing the switch from discrete, single-site binding (78.1 kJ mol-1)to cooperative two-molecule clusters (203.0 kJ mol-1) to be favorable, stabilized \qty{34}{\%} against displacements via hydration and favoring Grotthuss proton hopping with an extremely low barrier height (\qty{19.5}{kJ.mol^{-1}}). To irreversibly trap the remaining Leachable-Acid fraction, a scheme involving chemical phosphorylation of the membrane was investigated and modeled using high-level DFT, and classic molecular dynamics simulations were performed over 500 ps to evaluate the impact of replacing hydrogen bonds with a covalent phosphate monoester bridge: The hydrogen bond is transformed to a P-O-C phosphate monoester and the desorption energy barrier becomes \qty{+251.9}{kJ.mol^{-1}} at 353.15 K (a 3.3-fold increase) with maintenance of intrinsic P-OH acid activity. 
 In the end, this continuum finite element model built within COMSOL converted atomistic transport properties to macroscopic single cell scale and showed a perfectly linear, ohmic, polarization curve. It can be concluded that this entire approach was able to couple atomistic bonding and fuel cell level engineering, indicating that covalent phosphorylation represents an unambiguous synthetic pathway towards stable alternative proton exchange membranes.
...
Essentially, the world currently operates almost entirely on combustion. Whether this is an internal combustion engine, a gas turbine, or a coal power station, the fundamental chemistry remains: burn fuel, create carbon dioxide, recover some of the resultant energy as heat or work. This was adequate for the past two centuries but, at this point in time, the side effects-climate change, city air pollution, global dependence on finite fossil fuel reserves-are far too serious. Electrochemical conversion has to replace combustion in one of its various forms and the hydrogen fuel cell is arguably the most advanced technology for this. \cite{1,2}

A hydrogen fuel cell involves the separation of oxidation of hydrogen into two half-reactions at two electrodes. Hydrogen at the anode has electrons extracted from it, giving off protons. These electrons pass through the external circuit generating electrical work and also pass to the cathode where together with protons, which have traveled through the membrane, and oxygen it forms water. There are only water vapor emissions. There are no combustion emissions, so no carbon dioxide and no oxides of nitrogen. The energy conversion is not limited by the Carnot cycle the way that thermal engines are, hence theoretically, fuel cells could be substantially more efficient than internal combustion engines for a given fuel\cite{1,2}.

The heart of this system is the membrane located between the two electrodes. The proton conduction has to be efficient, i.e. Low membrane resistance leads to minimal voltage loss. The membrane must be an electron insulator and it also has to isolate the fuel gas from the oxidant gas. Finally, the membrane must be stable to thermal and humidity conditions at elevated temperatures and pressures, with no chemical degradation for thousands of hours. All these constraints lead to the proton exchange membrane (PEM) which is also known as the proton exchange material. \cite{3,4}

The dominant material that has existed in this field for several decades is Nafion; it is a perfluorosulphonic acid polymer manufactured by DuPont. It has excellent proton conduction abilities (over 100~mS\,cm\(^{-1}\) fully humidified) and high chemical stability to tolerate the corrosive atmosphere in fuel cells, \cite{5,6}. However Nafion is not technically challenging, but rather costly and environmentally hazardous; its production costs are greater than \$800 per square metre due to its complex multi-step synthesis of a fluorinated backbone. It represents over 40\% of the total cost of a fuel cell stack, \cite{1,4}.Besides cost, Nafion is a member of the class of per-and polyfluoroalkyl substances, which are a long-lived class of environmental contaminants and an increasing target of legislation both in Europe and North America.\cite{24} Additionally, Nafion performs poorly at anything above about 80\(^\circ\)C since it is mediated by liquid water within its channels and upon evaporation of the water above that temperature its conductivity declines rapidly, forcing complicated humidification apparatus to increase its weight, cost and failure points.\cite{3,4,5}

This limitation has resulted in twenty years of development for new alternative membranes made from cheaper and more available polymers. The aim is for an alternative that has similar conductivity to Nafion, can operate above 100°C without humidification, and has a cost that is a fraction of that of Nafion and is prepared without halogens. Not all of these conditions have yet been achieved with one material, but some areas are beginning to demonstrate realistic potential. PBI doped with phosphoric acid operates successfully above 100\(^\circ\)C without humidification but pure PBI is expensive and hard to process.\cite{20} 
Reasonable conductivity, and significantly lower cost than Nafion can be obtained with sulfonated polyetheretherketone, although increasing degree of sulfonation is associated with poor mechanical performance.\cite{18,19} Poly(vinyl alcohol) stands in a different region of design space: it is cheap, biodegradable, processed from water solution with good processability, and mass produced for other applications.\cite{9,10}

As such PVA alone cannot be employed as fuel cell membrane. With a hydroxyl group on practically every backbone carbon the PVA chain is infinitely water soluble, and the membrane will not form. The conventional method of engineer is to cross-link the PVA; citric acid is one of the more appealing crosslinkers since it is non-toxic and renewably based; citric acid is a hydroxyl reactive compound and can readily undergo esterification with PVA hydroxyls, no catalyst or solvent is necessary and temperatures around 150°C can be used to facilitate the reaction.\cite{50,51} 
The three carboxyl groups enable citric acid to act as a multi-chain polymer connector creating a covalent 3D network resistant to dissolution by any chemical means.\cite{48,53} Upon stabilization of the crosslinked film in water the membrane is then loaded with phosphoric acid by soaking in the acid. Phosphoric acid functions effectively as a proton conductor in the absence of free liquid water, for this reason it has also been chosen as the dopant for polybenzimidazole membranes.\cite{21,55} 
This combination of a low-cost and highly hydroxylated polymer (PVA), an environment friendly cross-linking agent (CA), and a proton conductor already developed (H\(3\)PO\(4\)) into one system exhibits the conductivities of 10-50 mS cm\(^{-1}\) at thermal stability up to 200\(^\circ\)C.\cite{9,12,13}

There are further applications associated with producing these membranes via electrospinning instead of by solution casting. In electrospinning, a polymer solution is forced through a charged needle and a solution into the fibers with an average diameter ranging from several hundred nanometers to a couple of micrometers, which then form a mat on a grounded collector\cite{37,38}.
The membrane produced therefore possesses a huge area relative to its volume in order to provide maximum available hydroxyl groups for the uptake of the acid and to provide short diffusion pathways for the protons to move through the membrane.\cite{40,41,42}
 The nanofibrous structure also affects the doping response: due to the absorption and swelling of the fibers as they are doped with acid, their progressive merge into a more dense material structure can be directly observed with a scanning electron microscope and correlated with the change in conductivity detected through an electrochemical impedance spectroscopy measurement.\cite{39,52}

Although PVA/CA/H\(_3\)PO\(_4\) membranes have technological relevance three fundamental questions concerning molecular mechanisms remains unanswered. One question that has not yet been solved is why conductivity increases non-linearly with doping time. A 30 min soak in phosphoric acid increases conductivity by a factor of 10 but doping up to 24 hours only increased it further by a factor of three. The relationship is linear if each phosphate molecule affects conductivity by a separate identical amount. However, this does not explain the observed relationship and no mechanistic reasoning is put forward.\cite{16,17}Secondly, does the proton transfer occur via the Grotthuss mechanism-where the proton jumps between the fixed chains of H-bonds in a pre-ordered structure, and the acid molecule does not diffuse through the membrane-or via the vehicular mechanism where the proton essentially travels through the membrane with the aid of the acid molecule's diffusion? The consequences of these mechanisms for the designing of the membrane are drastically different, however in a complex cross-linked polymer network they are experimentally very difficult to differentiate.\cite{16,17}The third question we pose is how 
only about 34\% of the incorporated phosphoric acid 
survives water washing. The acid enters, but most of 
the acid leaves. There's a portion left over, and this 
portion, has been reported for many years without any 
explanation at a molecular level, as to why 34\% of it 
and not 10\% or 
90\% remains.\cite{35,52}

There is no information which these experimental measurements on a macroscopic level can answer about these aspects. Proton transfer events take place on femtosecond times and within distance scales of a few Ångström, neither of which can be studied on a macroscopic level. Quantum-mechanical calculations are necessary to determine binding energies, transfer barriers and electronic structure changes. DFT has already successfully calculated these for Nafion and polybenzimidazole where the computations accounted for hydrational control of conductivity and calculated proton transfer barrier energies which correlated to experimental activation energies.\cite{18,19} No such study existed 
for PVA/CA/H\(_3\)PO\(_4\) before this work.

This dissertation bridges that gap. In here, electrospinning fabrication and experimental characterization of PVA/CA/H\(3\)PO\(4\) membranes are merged with 31 DFT calculations at B3LYP-D3BJ/6-31G* level of theory using ORCA 6.1.1.\cite{20,21}
 The first 23 calculations determine the molecular mechanisms of the experimentally manufactured system, which answer all three of the above questions. The latter eight calculations serve to explore in a direction that was made apparent by the experiment alone. That is, do the aforementioned problems go away if we swap the phosphoric acid to PVA hydrogen bond with a covalent P-O-C phosphate ester bond, and would conductivity be affected?
The covalently bound phosphate groups are dynamically confirmed as being structurally stable with a classical molecular dynamics at 353.15~K conducted using LAMMPS, which coupled to a COMSOL Multiphysics finite element model relates the transport characteristics at a molecular scale to device scale polarization behavior constitutes a three-tiered simulation structure; DFT, MD, and FEA, that span atom-scale bonding chemistry to fuel cell engineering for this type of alternate membrane material.\cite{18,19}
...
\subsection{Proton Transport Theory (Grotthuss vs.Vehicular)}
...
\section{Preparation and Rheological Characterization}
...
The dissolution was carried out by employing a BIOSAN MSH300 magnetic stirrer with a heat source.
...
Proton conductivity was determined by electrochemical impedance spectroscopy 
using a Biologic potentiostat controlled by EC‑Lab software. The membrane 
was sandwiched between two blocking stainless‑steel electrodes with a 
circular contact area of diameter $2\ \text{cm}$ ($A = 3.14\ \text{cm}^2$). 
Measurements were performed at $24^{\circ}\text{C}$ and $30\%$ relative 
humidity over the frequency range $100\ \text{kHz}$ to $10\ \text{mHz}$ with 
an AC amplitude of $10\ \text{mV}$ (rms). Prior to analysis, the forward 
frequency sweep ($100\ \text{kHz}\rightarrow 10\ \text{mHz}$) was isolated 
from any reverse sweep to avoid artefacts from hysteresis.

The raw impedance data ($Z^{\prime}$, $Z^{\prime\prime}$) were exported and 
analysed by two independent methods. First, the bulk membrane resistance 
$R_{b}$ was obtained from the high‑frequency intercept of the Nyquist plot 
($-Z^{\prime\prime}=0$) by linear interpolation of the two data points 
immediately adjacent to the crossing (for samples where the crossing fell 
within the measured frequency range), or by linear extrapolation of the 
three highest‑frequency data points ($100$--$50\ \text{kHz}$) to zero 
imaginary impedance (for samples where the intercept fell outside the 
experimental window).

Second, the full impedance spectrum was fitted to an equivalent circuit 
model consisting of a resistor in series with a constant‑phase element 
(R1~+~CPE1), using ZView~4 software (Scribner Associates, Inc.). 
This model represents the bulk membrane resistance $R_b$ (R1) and the 
non‑ideal capacitive response of the blocking electrode interface (CPE1). 
All three parameters (R1, CPE1‑T, CPE1‑P) were allowed to vary freely 
during the fit. The quality of fit was assessed by the chi‑squared value 
($\chi^{2}$) and by visual inspection of the superimposed experimental 
and fitted curves in both Nyquist and Bode representations.

The proton conductivity $\sigma$ was then calculated from the bulk 
resistance using
\begin{equation}
\sigma = \frac{L}{R_b \cdot A}
\label{eq:conductivity}
\end{equation}
where $L$ is the membrane thickness and $A$ the electrode contact area.
...
\ref{tab:covalent-calcs}. Geometry optimisations used TightOpt convergence 
...
Classical molecular dynamics (MD) simulations were conducted with LAMMPS(Large-scale Atomic/Massively Parallel Simulator, 29 September 2021) \cite{104}. The simulation box was constructed directly from the DFT optimized Calc07 geometry – a 52-atom cross-linked membrane consisting of 2 PVA monomers, a citric acid cross-linker, and 2 attached phosphate groups so that the MD starting structure completely agrees with the quantum mechanics reference. The force-field used for the membrane atoms was OPLS-AA(Optimised Potentials for Liquid Simulations, All-Atom) \cite{95}. The atom type was assigned according to the DFT connectivity calculation: the alkyl CH carbons as opls 135/137, the bridging oxygen of the phosphate groups as opls 480, the phosphorus atom as opls 479, the P(=O) oxygen as opls 481, the P--OH oxygen as opls 482, the carbonyl carbon of ester group as opls 235, the carbonyl oxygen of ester group as opls\_236, and the water was modeled using SPC/E model.\cite{107}, reproduces correctly self-diffusion coefficient and dielectric constant of liquid water at 353 K. 150 SPC/E water molecules were arranged randomly in the simulation box with the minimum distance 2.5~Å between any atoms to achieve a system of 547 atoms within a cubic periodic box of side 50.54~\AA{}. The pair interaction cutoff distance was set to 6.0~\AA{}, fulfilling the criterion that cutoff should be less than half the box length to eliminate periodic self-interaction.

\subsubsection{Force Field Validation}

Before any dynamics were performed the system was minimised and bond lengths compared to DFT reference values calculated using Calc01 (B3LYP-D3BJ/6-31G*/ORCA~6.1.1). The P--O--C bridge bond length converged to 1.623~\AA{} (DFT: 1.612~\AA{}, $+0.7$\%), the C--O--P carbon side bond length to 1.441~\AA{} (DFT: 1.465~\AA{}, $-1.6$\%), the P\(=\)O to 1.480~\AA{} (DFT: 1.475~\AA{}, $+0.3$\%), the phosphate O--H bond to 0.967~\AA{} (DFT: 0.972~\AA{}, $-0.5$\%). All bond lengths are within \(\pm\)0.05~\AA{} threshold expected for validation of OPLS-AA\cite{95} therefore verifying that the force field faithfully represents the DFT predicted covalent P--O--C structure.

\subsubsection{Equilibration and Production Protocol}

The system was brought to equilibrium via five successive NVT steps using a Nos\'{e}--Hoover thermostat along with a secondary velocity-rescaling fix (temp/rescale) applied every 10--100 steps, to avoid kinetic energy blowouts during heating: K from 1 to 10 using a timestep of 0.1~fs over 10~ps; K from 10 to 50 using a timestep of 0.2~fs over 10 ps; K from 50 to 150 using a timestep of 0.5~fs over 25~ps; K from 150 to 353.15~K using a timestep of 1.0 fs over 50 ps; and finally maintained at 353.15~K for 100~ps. The heating strategy used is required for polymer systems in which large velocity gradients present at the beginning of dynamics can be seen to destabilize intranolecular bond forces \cite{104}. After equilibrium the system was taken over 500~ps using 1.0~fs dynamics at 353.15~K, where atomic positions were recorded every 1~ps, and this data analyzed in terms of trajectories.

\subsubsection{Computed Quantities}
Two measures were computed from the production run: mean square displacement of the phosphorus atoms—calculated using \texttt{compute msd} taking centre-of-mass correction, time-averaged every 100 steps—and radial distribution function of P atoms with bridging oxygen atoms. 

A bounded MSD—i.e., one that shows no steady-state growth—reveals that the phosphate group is covalently tethered rather than diffusing, which is characteristic of acid retention. Free diffusion with subsequent leaching of the acid would instead yield a linearly growing MSD \cite{64}. The time-dependent MSD is calculated via:

\begin{equation}
\text{MSD}(t) = \langle |\mathbf{r}_i(t) - \mathbf{r}_i(0)|^2 \rangle = \frac{1}{N}\sum_{i=1}^{N} \Big[ (x_i(t)-x_i(0))^2 + (y_i(t)-y_i(0))^2 + (z_i(t)-z_i(0))^2 \Big]
\end{equation}

where $N$ refers to the number of tracking atoms, $\mathbf{r}_{i}(0)$ is for the initial unfolded configuration, while $\mathbf{r}_{i}(t)$ refers to the positions at simulation time $t$. The diffusion constant $D$ is obtained by plotting the slope of the MSD graph in steady-state using 3-D Einstein equation:
\begin{equation}
D = \frac{1}{6} \lim_{t \to \infty} \frac{d}{dt} \Big( \text{MSD}(t) \Big)
\end{equation}

The RDF, $g(r)$, between P atoms and bridging O atoms, using 100 bins and averaged over the full 500~ps production run with \texttt{compute rdf}, was analyzed to monitor the covalent P–O–C linkage integrity. The pair radial distribution function is expressed mathematically as:

\begin{equation}
g(r) = \frac{V}{N^2} \left\langle \sum_{i} \sum_{j \neq i} \delta(r - |\mathbf{r}_i - \mathbf{r}_j|) \right\rangle
\end{equation}

Here, $V$ is the volume of the simulation box, and the brackets indicates an ensemble averaging over the steps in the production run phase. Calculating the integral over the area under the first few coordination shells of $g(r)$ yields the localized coordination numbers. This cross-references the structural stability of the clusters, obtained by means of static quantum mechanical DFT calculations.
...
A 2-D single cell PEMFC model has been developed within COMSOL Multiphysics 6.3 using the Secondary Current Distribution interface, which includes the conservation of ionic and electronic current with Butler-Volmer electrode kinetics. The geometry comprises of a single membrane of size 100~\(\mu\)m 360~\(\mu\)m and boundaries of anode and cathode current collectors.
...
constant, and \(T = 297.15\)~K.

\subsubsection{Input Parameters and Limitations}

The membrane proton conductivity was set at $\sigma{m} = 41.4\ \mathrm{mS\,cm^{-1}}$, which was the experimentally determined value of the heat-treated membrane at $24^{\circ}\text{C}$ and $30\%$ RH (section~\ref{sec:conductivity}, table~\ref{tab:conductivity-results}). The membrane thickness was $L = 360\ \mu\text{m}$, which was the thickness of the membrane measured in the EIS experiment. This experiment-based input allows the COMSOL model to generate an perfectly linear polarization curve, the output membrane resistance was $0.278\ \Omega$ and agreed with the bulk resistance obtained from Nyquist intercept $Rb=0.276\ \Omega$ within 0.7% (section~\ref{sec:conductivity}), validating the consistency of the multiscale approach and negating the use of a literature value. The rest of the boundary conditions and solver options used are recorded in appendix F.
...
To establish a structural reference of the membrane series the statistics of the fiber diameter of the as spun thermally cross-linked S0 sample were calculated. Fig. 4.1 shows the distribution of diameters from several high-mag SEM images. The fibers are very uniform; the majority of diameters lies in the range of 200-300 nm. The average diameter of around 256 nm indicates the electrospinning conditions (voltage, flow rate) were appropriate for the viscosity of the PVA/CA solution. It is crucial for the function of the membrane that the diameter lies at this scale, thus forming a high specific surface area. For transport through the membrane, this fineness ensures that during subsequent doping, phosphoric acid has instant and uniform access to the polymer chains allowing rapid penetration into the amorphous regions of the PVA.


\subsection{Membrane Morphology Evolution — SEM Analysis}
The changes of the membrane morphology at the whole doping time span were investigated using Scanning electron microscopy. Two magnification levels of SEM images of samples were used in the experiment so that the large-scaled and microscopic structure at fibre/film level can be both observed. All the SEM images obtained in this experiment is illustrated as follows: Figure 4.1.\cite{39}
\subsection{S0 (Undoped Membrane): Pristine Nanofibrous Topology}
Starting from the S0 membrane which shows the state of the material after electrospinning and thermal curing, but before any acid exposure, the morphological development series starts from the S0 membrane. It's clear that the membrane has a typical non-woven "spaghetti-like" structure from the SEMs taken, Figure 4.2. The surface of the fibers is smooth and cylindrical without any 'beads' which are common in non-optimized electrospun polymers.

It is extremely important to note that the fibers remained distinct from each other and overlapping in a random fashion, giving a very porous, open, interlinked web structure. This proves that thermal crosslinking was achieved at 150$^\circ$C, esterifying the hydroxyl groups of PVA with Citric Acid carboxyl groups, without the fibers melting or becoming 'de-threaded', maintaining the 1D nature of the structures. The unaltered topology has inherent structural integrity as it offers the mechanical structure to support the initial plasticization that occurs when the doping begins, as detailed in the following section for samples S1-S3.
...
The proton conductivity of both membranes (immersedonly and immersed+heattreated) and the baseline undoped crosslinked PVA membrane was determined using electrochemical impedance spectroscopy. All experiments were conducted at $24^{\circ}\text{C}$ and $30\%$ RH. Section~\ref{sec:eis} gives an outline of the experimental setup, sample preparation and data analysis. A full list of fitting parameters of equivalent circuits as well as respective Nyquist and Bode plots can be found in the Appendix~\ref{app:eis}.
\subsection{Bulk Resistance and Equivalent Circuit Fitting}

The impedance spectra were fitted to an R 1 ~+~CPE 1 equivalent circuit using ZView~4. Both fits were obtained to a chi-squared of 4 or below, with R b =0.277 ~ W( ~ w 2=0.0047) for the heattreated membrane and R b =1.45 ~ W( ~ w 2=0.0065) for the immersed only membrane, within the error of the fitting parameters compared to bulk resistances interpolated from the high-frequency Nyquist intercept(0.276 ~ W and 1.42 ~ W respectively). The CPE exponents,, were 0.94 for the heattreated sample and 0.89 for the immersed only sample. The exponents are characteristic of an ideal blocking capacitive electrode.
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{fitted origin and zoomed with circuit for thesis.png}
\caption{Nyquist plots with R1~+~CPE1 equivalent circuit fits for the 
immersed‑only and immersed~+~heat‑treated PVA/H$_{3}$PO$_{4}$ membranes 
at $24^{\circ}$C, $30\%$ RH. Open symbols: experimental data. 
Solid lines: R1~+~CPE1 fits. Insets: magnified high‑frequency region 
showing the determination of the bulk resistance $R_{b}$.}
\label{fig:nyquist-combined}
\end{figure}

\subsection{Proton Conductivity Results}

From the bulk resistance that we obtain, we can calculate the proton conductivity using $\sigma = L/(R_{b}A)$, where L is the membrane thickness and $A=3.14\ \text{cm}^{2}$ is the area of contact between the electrodes. We list them on Table~\ref{tab:conductivity-results}.

\begin{table}[h!]
\centering
\caption{Proton conductivity at $24^{\circ}\text{C}$, $30\%$ RH 
(fitted R1~+~CPE1 values).}
\label{tab:conductivity-results}
\begin{tabular}{l c c c c}
\hline
Sample & $R_{b}$ ($\Omega$) & $L$ ($\mu$m) & 
$\sigma$ (S\,cm$^{-1}$) & $\sigma$ (mS\,cm$^{-1}$) \\
\hline
Pure PVA (crosslinked, undoped) & ${>}10^{3}$ & 100 & 
${<}3.2\times10^{-6}$ & ${<}0.003$ \\
PVA + H$_{3}$PO$_{4}$ (immersed) & 1.45 & 320 & 
$7.0\times10^{-3}$ & 7.0 \\
PVA + H$_{3}$PO$_{4}$ (immersed + heat‑treated) & 0.277 & 360 & 
$4.14\times10^{-2}$ & 41.4 \\
\hline
\end{tabular}
\end{table}

The as-synthesized membrane is almost an insulator ( $\sigma <0.003\ \text{mS\,cm}^{-1}$). The phosphoric acid absorption increased the conductivity to $7.0\ \text{mS\,cm}^{-1}$, and subsequent heat treatment leads to a six-fold increase to $41.4\ \text{mS\,cm}^{-1}$. A higher improvement obtained at equivalent phosphoric acid loading can be due to the phosphoric acid redistribution through heat treatment into collective binding states (Section:DFT-Cooperative) and formation of complete proton channels due to fusion of fibrous structures (Section:SEM).

\subsection{Comparison with Nafion and Literature}

Table~\ref{tab:conductivity-comparison} compares our data to that obtained previously. 
Under saturated humidity conditions, Nafion 117 achieves $90$--$100\ \text{mS\,cm}^{-1}$ but has low conductivity at low humidity. This means that the heat treated PVA/CA/H$_{3}$PO$_{4}$  membrane with an conductivity 
of ($41.4\ \text{mS\,cm}^{-1}$, $30\%$ RH) provides a viable alternative to Nafion under low humidity operation.

\begin{table}[h!]
\centering
\caption{Comparison of proton conductivity with literature values.}
\label{tab:conductivity-comparison}
\begin{tabular}{l l c c c}
\hline
Material & Doping / Treatment & $\sigma$ (mS\,cm$^{-1}$) & 
T ($^{\circ}$C) & Reference \\
\hline
Nafion 117 & Fully hydrated & 90--100 & 25 & \cite{5,6} \\
PVA/H$_{3}$PO$_{4}$ blend & Various & 10--40 & 25 & \cite{57,58} \\
PVA/sulfosuccinic acid & Crosslinked & 20--30 & 25 & \cite{113} \\
\hline
This work --- pure PVA & Undoped, crosslinked & ${<}0.003$ & 24 & --- \\
This work --- immersed & H$_{3}$PO$_{4}$, no HT & 7.0 & 24 & --- \\
This work --- immersed + HT & H$_{3}$PO$_{4}$ + heat‑treated & 41.4 & 24 & --- \\
\hline
\end{tabular}
\end{table}

\subsection{Note on Temperature‑Dependent Measurements}

Variable temperature EIS was not completed in this project, hence no Arrhenius activation energies are given. It is believed that room temperature conductivity data are adequate to compare to the DFT proton transfer barriers from Chapter 5.

\subsection{Non‑Linear Conductivity Enhancement: Molecular Interpretation}

This factor-of-six difference in conductivity between the immersed-only and heat-treated membranes, with very similar acid loading, highlights that it is the binding state of the acid that controls proton conduction, not its total amount. The effect of the heat treatment is that the acid is allowed to migrate from isolated single-site binding environments (binding energy: $-78.1$~kJ~mol$^{-1}$) to paired two-molecule groups (overall binding energy: $-203.0$~kJ~mol$^{-1}$), creating more effective short proton-relay steps and resulting in the superlinear increase in conductivity seen above. A clear connection between cooperative binding and macroscopic transport represents one of the main novel aspects of this work.
...
This cooperative effect provides the molecular basis for the dramatic conductivity enhancement observed experimentally. 
Immersion of the crosslinked PVA/CA membrane in 30~wt\% H\textsubscript{3}PO\textsubscript{4} without further treatment yields a conductivity of 7.0~mS\,cm\textsuperscript{\(-\)1}, while an identical membrane subjected to heat treatment at 150\textdegree C after immersion reaches 41.4~mS\,cm\textsuperscript{\(-\)1} (Section~4.4.3). 
Despite a comparable total acid loading, the heat‑treated membrane achieves a factor‑of‑six higher conductivity. 
The DFT cooperative binding analysis explains this result: thermal energy during heat treatment allows phosphoric acid molecules to redistribute from isolated single‑site binding positions (binding energy $-78.1$~kJ~mol$^{-1}$) into cooperative two-molecule assemblies (combined binding energy $-203.0$~kJ~mol$^{-1}$). 
Each additional acid molecule entering a cooperative configuration binds more strongly and forms shorter, more efficient proton relay contacts with lower effective transfer barriers, producing a superlinear conductivity gain. 
The same cooperative mechanism is consistent with the non‑linear conductivity progression with doping time reported in previous experimental studies of PVA/H$_3$PO$_4$ membranes \cite{57,58}.
...
This 37.8 kJ/mol peak is accessible at room temperatures, since \(kT\) at 25\(^\circ\)C is only about 2.5 kJ/mol. As the temperature is increased to 60-80\(^\circ\)C, this peak becomes easier to traverse, consistent with activation energies of $10$--$21$~kJ~mol$^{-1}$ reported for comparable PVA/H$_3$PO$_4$ systems in the literature [57, 58, 60].
...
From the data presented in Table 5.8 and Figure 5.8, the value of \(\Delta G^{\ddagger}\) decreases from 19.5 kJ/mol at 25\(^\circ\)C to 14.1 kJ/mol at 130\(^\circ\)C. This is due to the increasing vibrational entropy of the flexible hydrogen-bond network as the temperature is increased, making the transition state more accessible with increasing temperature. This trend is qualitatively consistent with the experimentally observed conductivity improvement following heat treatment (Section~4.4), and with activation energies of $10$--$21$~kJ~mol$^{-1}$ reported in the literature for comparable PVA/H$_3$PO$_4$ systems~\cite{57,58,60}.
...
\midrule
\multicolumn{5}{l}{\footnotesize Literature range for phosphoric-acid-doped polymer membranes:} \\
\multicolumn{5}{l}{\footnotesize \hspace{1em} PBI/H\(_3\)PO\(_4\) DFT barriers 10--40~kJ\,mol\(^{-1}\) \cite{20,59}; experimental \(E_a\) for PVA/H\(_3\)PO\(_4\) systems 10--21~kJ\,mol\(^{-1}\) \cite{57,58,60}.} \\
\bottomrule
\end{tabular}
\end{table}

This energy barrier of 19.5 kJ/mol calculated by the CalcNew3 method falls within the range of activation energies of $10.2$--$20.9$~kJ~mol$^{-1}$ reported experimentally for PVA/H$_3$PO$_4$ membranes [57, 58, 60]. Two entirely different techniques have found the same energy barrier for the transport of protons. This is the major result of the entire calculation effort.\cite{65, 66, 72, 73}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\linewidth]{DFT PREDICTED SPECTRUM OF CROOSSLINKED WET MEMBRANE.png}
...
The crosslinked CalcNew3 cluster model incorporates two H\(_3\)PO\(_4\) molecules. This structural arrangement positions its local acid density between that of the brief surface adsorption state (S1) and the fully saturated matrix state (S3). The calculated minimum-energy proton-transfer barrier of 19.5 kJ/mol provides a physically realistic baseline for an intermediate doping state, modeling the network continuity prior to complete thermal fusion \cite{18, 19}.

\section{Post-Treatment Conductivity and Thermodynamic Correlation}
The macroscopic six-fold conductivity increase directly validates the microscopic DFT model. Without heat treatment, the immersed membrane achieves a baseline conductivity of 7.0 mS cm\(^{-1}\). In this non-activated state, phosphoric acid occupies isolated, single-site binding positions. DFT calculates this baseline isolated binding energy at \(-78.1\) kJ mol\(^{-1}\).

Subsequent thermal activation drives a superlinear jump to 41.4 mS cm\(^{-1}\). Heat provides the energy required to rearrange the acid molecules into cooperative, two-molecule assemblies. The combined cooperative binding energy reaches \(-203.0\) kJ mol\(^{-1}\). This thermodynamic driving force alters the membrane morphology. SEM analysis confirms a complete transition into a fused, dense film. High-resistance fiber junctions fully collapse. This fusion creates continuous Grotthuss proton-relay networks. High-affinity surface states lower the water contact angle to 50.6\(^{\circ}\). Macroscopic transport trends match the calculated molecular configurations.

\subsection{Cooperative Binding and Room-Temperature Conductivity Enhancement}
The non-linear conductivity trend validates the cooperative binding model. Short-term immersion yields isolated surface adsorption where acid molecules occupy single-site states, configuring a baseline conductivity of 7.0 mS cm\(^{-1}\). Extended doping promotes deep matrix penetration and drives cooperative two-molecule network formation. 

Thermal treatment accelerates this molecular relocation. The acid shifts to highly stable configurations where the combined binding energy reaches \(-203.0\) kJ mol\(^{-1}\). This shift triggers the six-fold conductivity leap to 41.4 mS cm\(^{-1}\). SEM tracks a complete morphological shift where nanofibrous networks coalesce into dense films, fiber boundary resistance collapses completely, and continuous Grotthuss proton-relay pathways form throughout the bulk matrix.

\textbf{Mapping Binding Energies onto Conductivity Values}

The experimental conductivity measurements revealed a factor-of-six enhancement between the immersed-only membrane (7.0~mS\,cm$^{-1}$) and the heat-treated membrane (41.4~mS\,cm$^{-1}$) at a comparable total acid loading. This superlinear gain cannot be explained by an increase in absolute acid quantity alone. The DFT binding energies provide the molecular explanation: the first phosphoric acid molecule binds to an isolated hydroxyl site at \(-78.1\)~kJ\,mol$^{-1}$.

When a second molecule binds cooperatively at a neighboring site, the combined binding energy reaches \(-203.0\)~kJ\,mol$^{-1}$, corresponding to \(-101.5\)~kJ\,mol$^{-1}$ per molecule. This represents a 30\% stabilization enhancement relative to the isolated case. These cooperatively bound molecules participate in shorter, stronger hydrogen bonds that lower effective proton transfer barriers and create efficient relay chains, producing a conductivity that increases significantly faster than the absolute carrier density \cite{21,55,100}.

\textbf{Molecular Explanation for Non-Linear Enhancement}
The first acid molecules incorporated during the initial 30 minutes form isolated, single-point attachments. As doping progresses, subsequent acid molecules bind cooperatively adjacent to the existing sites. These adjacent configurations optimize multi-point binding states with localized cluster energies ranging from \(-156\) to \(-203\) kJ/mol. This clustering maximizes chain efficiency due to close geometric proximity with neighboring PVA hydroxyl groups. Consequently, extended doping time increases the net population of active proton-transfer contacts, ensuring each contact participates in a shorter transport pathway with a minimized local potential barrier \cite{21, 55, 100}.
...
Analytical phosphorus measurements indicate that 34\% of the total incorporated H\(_3\)PO\(_4\) is firmly retained within the material structure after exhaustive water washing. Prior to this study, this value was treated as an arbitrary empirical observation lacking a clear molecular mechanism. 

The DFT two-population model, derived from the desorption potential energy surface (PES) and hydration stability calculations, provides the physical basis for this 34/66 split. Acid molecules bound at single-site positions (\(-78.1\) kJ/mol) are thermodynamically vulnerable to competitive hydration displacement, causing 66\% of the initial loading to leach out. Conversely, the 34\% fraction bound in cooperative multi-point configurations (\(-156\) to \(-203\) kJ/mol) exhibits a desorption barrier that aqueous competition cannot cross at operating temperatures, successfully anchoring the acid species within the matrix \cite{63, 64}.

\textbf{Parameter-Free Prediction}
This structural division is predicted directly from first-principles energetic thresholds without empirical parameter fitting. Utilizing static quantum-chemical stability domains, the model naturally replicates the two-population experimental outcome of 34\% retention and 66\% loss. This parity validates the computational framework as an predictive tool rather than a post-hoc rationalization \cite{63, 64, 100}.

\subsubsection{Three Independent Proofs of the Grotthuss Mechanism}
\textbf{Proof 1 — Energetic (PES and NEB)}
The crosslinked wet membrane model exhibits a minimum-energy proton transfer barrier of 19.5 kJ/mol \cite{89,90,91}. This low energy threshold is a classic indicator of Grotthuss hydrogen-bond hopping, requiring minimal thermal activation at room temperature. In contrast, a vehicular transport mechanism would require the physical diffusion of a 98 Da phosphoric acid species through a dense, crosslinked polymer network. Overcoming the steric and viscous resistance of this crosslinked matrix would dictate a substantially higher activation barrier, confirming that transport is dominated by structural relay pathways rather than molecular translation.

\textbf{Proof 2 — Electronic (NBO Bond Order)}
At the calculated transition state geometry, Natural Bond Orbital (NBO) analysis shows a shared proton bond order of 0.7455. This electronic distribution indicates that 25.45\% of the proton charge density has migrated toward the acceptor oxygen while maintaining significant coordination with the donor atom \cite{84,85}. This highly delocalized, symmetric sharing configuration matches the structural transition state of a Grotthuss hop. A vehicular mechanism would instead retain a localized covalent bond order near 1.000 on the carrier vehicle until a sudden interfacial transfer event occurred.

\textbf{Proof 3 — Topological (ELF)}
The transition state Electron Localization Function (ELF) map reveals two sharply focused maxima exceeding 0.85 centered on the core donor and acceptor oxygen atoms. These localized valence basins are connected by an electron-deficient channel where the ELF value falls below 0.30 directly along the proton trajectory. This topology represents the classic electrostatic signature of Grotthuss relay transport, characterized by an electron-poor pathway running between distinct lone-pair regions. Vehicular diffusion would instead show a continuous, high-density valence shell moving alongside the proton core.
...
The radial distribution function between phosphorus and bridging oxygens g(r) has been calculated during the entire 500~ps production phase at 353.15~K. It is displayed in Figure~5.18. The radial distribution function g(r) shows a clearly defined first peak at $r \approx 4.7-4.8$~\AA{}. This first peak remains unmodified for the entire simulation time. It does not display any signal for $r < 1.5$~\AA{}, which confirms the absence of atom overlap in the system.

It is important to study the position of this peak with care. The covalent bond P--O in the P--O--C bridge has length 1.623~ \AA{} (Section~5.1) and the neighboring C--O bond has length 1.441~\AA{}. The P is thus located about 2.7--3.1~\AA{} away from the backbone carbon and about 4.7--4.8~\AA{} from the next-neighbour O atoms on the polymer chain. The peak position in the RDF found corresponds precisely to this range, proving that the environment of the phosphate group in the MD simulation matches that predicted by the DFT-optimized structure. Crucially, the fact that this peak does not move and maintains its shape throughout a 500~ps trajectory at 353~K shows that the P--O--C covalent structure is maintained on this timescale at the operating temperature of a fuel cell without bond reordering or drift. The kinetic information, obtained in addition to the thermodynamic DFT result, shows that a 251.9~kJ\,mol\(^{-1}\) desorption barrier for the phosphate does not simply reflect a theoretical minimum in energy but the actual stable existence of the C-O-P bridge on the nanosecond timescale achievable using classical simulation \cite{105}.
...
In Fig.5.19, the MSD of the phosphorus atoms from the 500~ps production at 353.15~K is given. It can be seen that the P-atom MSD is always contained in the shown range and it oscillated between 0.05--0.83~\AA\(^2\), having average value at 0.18~\AA\(^2\) with the final value of 0.83~\AA\(^2\) at 500~ps.

The physical meaning of the present result is illustrated well by a comparison. A freely diffusing phosphoric acid molecule at 353 K would have an MSD increasing linearly with time in the Einstein relation: $\langle r^2 \rangle = 6Dt$ and, given a common diffusion coefficient of $D \sim 10^{-9}$ m\(^2\)s\(^{-1}\) for a small molecule, would have reached some tens to hundreds of \AA\(^2\) in 500 ps [64]. Our observed MSD is three orders of magnitude less than this free diffusion value (and there is no linear dependence at all) which is direct proof of the covalent immobilization of the phosphate groups on the polymer backbone. The phosphoric acid groups thus only undergo small vibrations and libration around their equilibrium sites.

The two methods are in full agreement. As DFT predicts breaking the P-O-C bond costs $-251.9$~kJ~mol$^{-1}$), MD demonstrates the bond will not break in nanoseconds at 353.15 K. The two combined explain the behavior of the covalently phosphorylated membrane retaining acid when an H-bonded system would have a barrier of just $75.9$~kJ~mol$^{-1}$) (106).
...
The COMSOL Multiphysics model described in Section~3.7.12 was re‑parameterised 
with the experimentally determined proton conductivity of the heat‑treated 
membrane, $\sigma_m = 41.4\ \mathrm{mS\,cm^{-1}}$ (Section~4.4, Table~4.1). 
All other model parameters remain as listed in Appendix~F. The results below 
therefore represent the experimentally validated device‑scale simulation.

\subsection{Electrolyte Potential Distribution and Current Density Mapping}

The distribution of electrolyte potential is shown along with the streamlines for the current density through the membrane domain for a cell potential of $0.6 V$. The potential surface map color ranges between around -0.50 V on the left (cathode) and -0.10 V on the right (anode). It can be seen that the potential drops monotonically through the $360\,\mu m$ of membrane. The stream-lines are purely vertical and uniformly distributed across the entire $100\,\mu m$ of the membrane width without any tendency for lateral deviation or crowding.

This uniform flow field provides strong confirmation that the membrane proton transport is Ohmic-dominated and that the conductivity is laterally homogeneous. If there had been any lateral variations in the acidity of the material or any localized swelling, or if the fibres did not fuse completely during their manufacture, then lateral current gradients would have been observed- their absence clearly indicates the fully connected proton conducting network that the DFT and MD simulations represent on the molecular level \cite{66,111}.

\begin{figure}[h!]
\centering
\includegraphics[width=0.75\linewidth]{Untitled4.png}
\caption{Electrolyte potential (V) and current density streamlines across the 
PVA/CA/H$_3$PO$_4$ membrane domain at $V_\text{cell}=0.6$~V. The input 
conductivity is the experimentally measured value for the heat‑treated membrane 
($41.4$~mS\,cm$^{-1}$).}
\label{fig:comsol-potential}
\end{figure}

\subsection{Polarisation Curve and Cell Potential Response}
The polarisation curve obtained from the COMSOL model is shown in Figure 5.21, and the corresponding corrected numerical values are listed in Table 5.9. The total cell current increases monotonically and linearly from 0.72~A at \(V_{\text{cell}} = 0.2\)~V to 3.60~A at \(V_{\text{cell}} = 1.0\)~V. Over the entire voltage range, the current–voltage relationship is strictly linear (\(R^2 > 0.999\)), which is characteristic of a pure ohmic conductor.

\begin{table}[h!]
\centering
\caption{COMSOL-computed cell voltage and total current for the PVA/CA/H\(_3\)PO\(_4\) membrane model at 297.15~K. Membrane conductivity input: \(\sigma_m = 41.4\text{ mS cm}^{-1}\), membrane thickness 360~\(\mu\)m, electrode area 3.14~cm\(^2\).}
\label{tab:comsol-polarisation}
\small
\begin{tabular}{c c}
\toprule
Cell Voltage, \(V_{\text{cell}}\) (V) & Total Current, \(I_{\text{cell}}\) (A) \\
\midrule
0.20 & 0.72 \\
0.30 & 1.08 \\
0.40 & 1.44 \\
...
\caption{COMSOL-predicted polarisation curve of the PVA/CA/H\(_3\)PO\(_4\) membrane cell.}
\label{fig:comsol-iv}
\end{figure}

The effective membrane resistance extracted from the slope of the polarisation curve is:
\begin{equation}
R = \frac{\Delta V}{\Delta I} = \frac{1.0\text{ V} - 0.2\text{ V}}{3.60\text{ A} - 0.72\text{ A}} = \frac{0.80\text{ V}}{2.88\text{ A}} \approx 0.278\ \Omega
\end{equation}

The calculated value here matches very closely the bulk resistance Rb=0.276 W calculated from directly from the experimental Nyquist plot (Section 4.4) (less than 1% error). The excellent agreement between these two points and from the experimentally determined (and calculated from EIS and independent physical thickness measurement) conductivity of the membrane, indicates that both values feed properly into the continuum simulation and that the device-scale output from COMSOL truly is simply the one-dimensional ohmic performance (ref 108,111).


\section{Integrated Multiscale Correlation}

These three computational levels provide a selfconsistent image of PVA/CA/H\textsubscript{3}PO\textsubscript{4} membrane. The DFT level defines an energy balance: P--O--C covalent bond shows $251.9\ \mathrm{kJ\,mol^{-1}}$ as its desorption barrier (acid loss impossible, thermodynamically speaking) while the $38.2\ \mathrm{kJ\,mol^{-1}}$ barrier for proton transfer for hydrated crosslinked membrane is in accordance to Grotthuss hopping process. These conclusions are confirmed by MD simulation on dynamical scale, as the Patom MSD remains <$1\ \mathrm{\AA^2}$ at $353\ \mathrm{K}$ during $500\ \mathrm{ps}$ and shows it is covalently immobilized, and the RDF profile, with peaks maintained at $4.7$--$4.8$\ \AA, illustrates stable structure of P--O--C connection.

The calculated transport properties are translated to device scale by COMSOL. Using the experimentally measured proton conductivity of the heattreated membrane (41.4 mS cm-1 Section 4.4), the model predicts an absolutely linear polarisation curve and current distribution. The calculated effective membrane resistance from the COMSOL polarisation curve (0.278 0) agree with the direct bulk resistance measured using EIS (0.276 0) to within 0.7 %. This excellent correlation implies that the molecular information extracted from the DFT and MD calculation -- cooperative acid binding, Grotthussdominated proton hopping, and nonmigrating acids as they were tethered – are correctly scaled to the bulk continuum model.

Each level is a source of the next. MD structural stability arises from the DFT bonding energy, which underlies the experimentally determined conductivity that influences COMSOL device behavior. This hybrid approach shows multiscale modelling is a viable strategy for linking QM theory with engineering parameters of relevance to the fuel cell, for this category of alternative membranes. \cite{64}

\chapter{Conclusions and Future Work}

\section{Summary of Principal Findings}

In this dissertation we sought answers to 3 questions that only experimental characterisation was unable to resolve, why H$_3$PO$_4$ binds to PVA/CA, how protons travel through the PVA/CA and why only approximately a third of incorporated acid remains after water wash. Thirty one DFT calculations at B3LYP-D3BJ/6-31G* using ORCA~6.1.1 alongside classical molecular dynamics and finite element modeling allowed answers to be obtained for all three.

The crosslinking is a thermodynamically favorable process ($\Delta E = -12.4$~kJ\,mol\(^{-1}\)) and the predicted DFT C=O ester frequency at 1720 cm\(^{-1}\) perfectly agrees with the FTIR-ATR experimental measurement. This agreement with no fitting parameters used is the first indication of correct calibration of DFT for this material system.

The phosphoric acid binds to PVA hydroxyl groups with an energy of $-78.1$~kJ\,mol\(^{-1}\), the hydrogen bond formed is only a single and a rather strong one for an O--H$\cdots$O contact in water. However the most interesting is when a second acid molecule binds. Both molecules are not binding independently but their binding energy is increased to $-203.0$~kJ\,mol\(^{-1}\) (which is 46.8~kJ\,mol\(^{-1}\)  more than a simply addition) because the first molecule affects the binding environment of the second and vice-versa. The effect described above is the origin at the molecular level of the non-linear enhancement in conductivity as the doping time increases from 30 min to 24 hours, which has been measured.

We characterized proton transfer via five different and independent calculation methods that all converge to the same value. 37.8~kJ\,mol\(^{-1}\) is predicted by the gas-phase PES scan, which was verified to represent a saddle point on the PES at $\nu^{\ddagger} = -1248$~cm\(^{-1}\) by the transition state optimization calculation. The Nudged Elastic Band calculation is found to reproduce the exact same barrier at the minimum energy path between these two species. At the transition state, the NBO bond order of the donor hydrogen to the donor oxygen is 0.7455, meaning 25.45\% of the proton has already moved to the acceptor oxygen while the original bond has only 74.55\% integrity remaining. ELF mapping at the transition state geometry also shows an electron depleted channel existing between the lone pairs on the donor and acceptor oxygens - indicating a Grotthuss hop instead of a vehicular hop. In a 65-atom crosslinked wet model, the barrier height decreased to 19.5~kJ\,mol\(^{-1}\).

 The question of acid retention was solved using two complimentary computations. Hydration stability gave $\Delta G = -278.0$~kJ\,mol\(^{-1}\) for H\(3\)PO\(4\)/water/PVA complex, and therefore that water stabilized the network of cooperative binding rather than destabilized it. The 20 step desorption PES ($+75.9$~kJ\,mol\(^{-1}\)) shows two populations, single sites that water has unbound (66\%) and sites bound cooperatively to which water cannot bind at room temperature (34\%), and matches without any empirical fitting.

The eight covalent phosphorylation calculations push the scenario in an interesting direction, practically relevant. The transition of hydrogen bond to P-O-C covalent linkage changes the desorption barrier from $+75.9$ to $+251.9$~kJ\,mol\(^{-1}\)-3.3 times. Meanwhile, the bond order of O-H on the rest P-OH protons donor groups slightly changes by 0.007 (from 0.745 to 0.752), the proton transfer barrier changes by 0.4~kJ\,mol\(^{-1}\) (from 37.8 to 38.2~kJ\,mol\(^{-1}\)). Those two parameters determining the acid retention and proton conductivity become contrasting with covalent linking; in the former the retention becomes significantly better, in the latter there is nearly no change.

Classical molecular dynamics at 353.15~K for 500~ps proved the DFT predictions dynamically. The mean square displacement of the P atom was consistently under 1~\AA\(^2\), three orders of magnitude lower than what free diffusion would predict, and the radial distribution function peak at 4.7--4.8~\AA{} was still present with no degradation observed, proving the P–O–C bond remained structurally intact at fuel cell temperature.

The COMSOL finite element model, parameterised with the experimentally measured conductivity of the heat-treated membrane ($\sigma_m = 41.4$~mS~cm$^{-1}$, Section~4.4), yielded a strictly linear polarisation curve ($R^2 > 0.999$) and uniform current density streamlines. The effective membrane resistance from the model ($0.278~\Omega$) agrees with the EIS Nyquist intercept ($0.276~\Omega$) to within $0.7\%$, completing the multiscale validation loop.
...
The 23.4~kJ\,mol\(^{-1}\) per-molecule cooperative 
...
All DFT calculations employ finite molecular cluster models in the gas-phase. The actual membrane is a condensed phase solid having long-range electrostatics, thermal disorder and an complicated dielectric environment, none of which is represented in a cluster model. Absolute binding energies and transfer barriers in the real system are certainly going to differ-the influence of solvation and dielectric screening is to generally decrease both from their gas-phase values. While the excellent agreement between the CalcNew3 barrier (19.5 kJ mol(-1)) indicate that the 65 atom model captures the most important effects of the environment, one can by no means conclude that the cluster perfectly models the membrane.

Calculating the frequencies from CalcNew2 resulted in a single tiny imaginary mode at -4.12 cm-1. This is a known numerical artifact of mobile hydrogen bonded networks, and it is not a true saddle point. It introduces an uncertainty of minor importance to the thermochemistry values calculated using this method, but has no bearing on the reported barriers or binding energies.

The model structure used is basic; the building block is a PVA molecule dimer cross-linked with a single citric acid molecule. Actual membrane consists of very high molecular weight polymer chains that each have many cross-links per chain and the membrane as a whole has a distribution of cross-link densities. Local chemistry can be accurately depicted but dynamics in terms of the long-range polymer behavior and long-range differences in cross-link density that might have an impact on proton transport beyond the scale that can be accessed in this model are beyond the scope of this methodology.
...
The closest subsequent experiment to the covalent phosphorylation calculations is experimental itself. Phosphorylated PVA films have been produced by phosphorylation with phosphoryl chloride in pyridine and H\(_3\)PO\(_4\). The main measurements would be an acid retention test after washing in water; where a value of close to 100\% would be expected from DFT against 34\% expected from conventionally doped membrane; and the EIS activation energy, where the value would be predicted to be around 38~kJ\,mol\(^{-1}\) if Grotthuss hopping remained after covalent binding.

\subsection*{Higher basis set and implicit solvation}

A single point energy calculation at def2-TZVP level on CalcNew2 geometry would be valuable for the comparison with gas phase binding energies reported in chapter~5. Adding implicit solvation with CPCM or SMD would reduce further the gas phase barriers compared with the experimental activation energy and would lower the value for the S3 doping level, where literature reports $E_a$ values as low as $10.2$~kJ~mol$^{-1}$ [57, 58, 60]
\subsection*{Ab initio molecular dynamics}

Although we calculate the potential energy surface via static DFT, this yields no information on dynamics. AIMD at the BLYP-D3/DZVP level would yield proton diffusion coefficients comparable directly to conductivity measurements, hopping rates at various temperatures, and an image of how the hydrogen-bond network fluctuates about the static configurations that we compute here. With 65 atoms CalcNew2 is small enough to fit into a university computing cluster.

\subsection*{Periodic DFT}

If the clusters were applied to a periodic unit cell of the crosslinked PVA/CA network then long-range electrostatic effects could be accounted for and surface artifacts of finite clusters avoided. This is computationally intensive but possible with plane-wave DFT codes (e.g. VASP) running on cluster hardware available.

\subsection*{Larger MD system for mechanical properties}

The current 547 atom periodic box is adequate for analysis of structures and transport, but the box size is too small for a statistically reliable stress strain calculation. A simulation performed on a larger periodic supercell, of between 5-10 times the size of the current system, would allow for calculation of a statistically sound Young's modulus of the covalently phosphorylated membrane in order to compliment the DFT predictions of retention and conductivity with analysis of mechanical stability.
...
The 65-atom CalcNew2/CalcNew3 calculation is the largest proton transfer DFT calculation for any PVA membrane, and the experimental measurement of 34\% acid retention matches quantitatively the parameter-free prediction of 34\%. The covalent phosphorylation study also provides a concrete synthetic goal: a membrane where the barrier to desorption is 3.3-fold greater than the present one and proton transport is almost unaffected.

None of the above required more than a four-core laptop and free software. The cluster DFT calculation approach if handled properly and cross-referenced at every step with experimental results will provide both quantitative predictions and a mechanistic explanation beyond that achievable with bulk characterization techniques. Whether the covalently phosphorylated membrane is a perfect system or not the framework built within this thesis – linking esterification energetics via cooperative binding via proton transfer barrier via device polarization curve – provides a robust foundation for future rational optimization of such a material class.
...
11 & 2\(\times\)H\(_3\)PO\(_4\) + PVA cooperative & 26 & Opt & \(\Delta E = -203.0\) kJ/mol (+23.4 cooperative) & \(\sim\)2 h \\
...
\section{Equivalent Circuit Model and Fitting}
\label{sec:D-equivalent-circuit}

As described in Section~\ref{sec:eis}, the impedance spectra were fitted 
to the R1~+~CPE1 equivalent circuit using ZView~4. The complete data sets, 
comprising both the forward and reverse frequency sweeps, were used in the 
fitting. The fitted parameters and goodness‑of‑fit metrics are summarised 
in Table~\ref{tab:D-fitted-params}; the corresponding bulk resistances and 
conductivities are reported in Table~\ref{tab:conductivity-results} 
(Section~\ref{sec:conductivity}).

\begin{table}[h!]
\centering
\caption{ZView fitted parameters for the R1~+~CPE1 equivalent circuit 
at $24^{\circ}\text{C}$, $30\%$ RH.}
\label{tab:D-fitted-params}
\begin{tabular}{l c c c c}
\hline
Parameter & \multicolumn{2}{c}{PVA + H$_{3}$PO$_{4}$ (immersed)} & 
\multicolumn{2}{c}{PVA + H$_{3}$PO$_{4}$ (immersed + heat‑treated)} \\
\hline
R1 ($\Omega$)               & 1.45   & ($\pm6.3\%$) & 0.277  & ($\pm6.8\%$) \\
CPE1‑T (F\,s$^{\alpha-1}$)   & $2.79\times10^{-5}$ & ($\pm3.5\%$) & $9.91\times10^{-5}$ & ($\pm2.7\%$) \\
CPE1‑P ($\alpha$)            & 0.89   & ($\pm0.65\%$) & 0.942  & ($\pm0.48\%$) \\
$\chi^{2}$                   & \multicolumn{2}{c}{0.0065} & \multicolumn{2}{c}{0.0047} \\
Sum of squares                & \multicolumn{2}{c}{0.176}  & \multicolumn{2}{c}{0.128} \\
\hline
\end{tabular}
\end{table}

The low $\chi^{2}$ values and the excellent overlay of the fitted curves 
on the experimental data (Figs.~\ref{fig:D-nyquist} and \ref{fig:D-bode}) 
confirm that the R1~+~CPE1 model adequately describes the blocking‑electrode 
response of both membranes.

\section{Nyquist Plots}
\label{sec:D-nyquist-plots}

Figure~\ref{fig:D-nyquist} shows the Nyquist plots with the fitted 
R1~+~CPE1 curves superimposed. The insets magnify the high‑frequency 
region where the bulk resistance is determined. Both manual interpolation 
and the ZView fit give consistent values (Section~\ref{sec:conductivity}).

\begin{figure}[h!]
\centering
\begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{NYQUIST PLOT.jpg}
    \caption{PVA + H$_{3}$PO$_{4}$ (immersed)}
    \label{fig:D-nyquist-immersed}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{nqusit ht.jpg}
    \caption{PVA + H$_{3}$PO$_{4}$ (immersed + heat‑treated)}
    \label{fig:D-nyquist-heattreated}
\end{subfigure}

\caption{Nyquist plots with R1~+~CPE1 equivalent circuit fits for the two 
conductive PVA/H$_{3}$PO$_{4}$ membranes at $24^{\circ}$C, $30\%$ RH. 
Open symbols: experimental data; solid lines: R1~+~CPE1 fits. Insets show 
the high‑frequency intercept region used to determine $R_b$.}
\label{fig:D-nyquist}
\end{figure}
...
\section{Bode Plots}
\label{sec:D-bode-plots}

The Bode plots (|Z| and phase angle) are presented in 
Figure~\ref{fig:D-bode}. The flat high‑frequency plateau in |Z| equals 
$R_{b}$, and the phase approaches $-90^{\circ}$ at low frequencies, 
characteristic of a capacitive blocking electrode. The close agreement 
between the experimental data and the fitted R1~+~CPE1 curve across the 
entire frequency range further validates the chosen equivalent circuit.

\begin{figure}[h!]
\centering
\begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{BODE PLOT1.jpg}
    \caption{PVA + H$_{3}$PO$_{4}$ (immersed)}
    \label{fig:D-bode-immersed}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{BODE PLOT (1).jpg}
    \caption{PVA + H$_{3}$PO$_{4}$ (immersed + heat‑treated)}
    \label{fig:D-bode-heattreated}
\end{subfigure}

\caption{Bode plots ($|Z|$ and phase angle) with R1~+~CPE1 equivalent circuit fits 
for the two conductive PVA/H$_{3}$PO$_{4}$ membranes at $24^{\circ}$C, $30\%$ RH. 
Points: experimental data; solid lines: R1~+~CPE1 fits.}
\label{fig:D-bode}
\end{figure}
...
Membrane thickness & 360~\(\mu\)m (heat‑treated sample) \\
Membrane width & 100~\(\mu\)m \\
Temperature & 297.15~K \\
Membrane conductivity & 41.4~mS\,cm\(^{-1}\) (experimentally measured, 
                          Table~\ref{tab:conductivity-results}) \\
...
where \(\sigma_m = 41.4 \times 10^{-3}\)~S\,cm\(^{-1}\) is the 
experimentally measured proton conductivity of the heat‑treated 
membrane (Section~\ref{sec:conductivity}) and \(\phi_m\) is the 
...
with exchange current density \(i_0 = 1 \times 10^{-5}\)~A\,m\(^{-2}\), 
symmetric transfer coefficients \(\alpha_a = \alpha_c = 0.5\), 
Faraday's constant \(F = 96{,}485\)~C\,mol\(^{-1}\), gas constant 
\(R = 8.314\)~J\,mol\(^{-1}\)K\(^{-1}\), and temperature 
\(T = 297.15\)~K (Table~\ref{tab:bv-params}).

\begin{table}[h]
\centering
\caption{Butler--Volmer kinetic parameters.}
\label{tab:bv-params}
\begin{tabular}{lll}
\toprule
Parameter & Symbol & Value \\
\midrule
Exchange current density & $i_0$ & $1 \times 10^{-5}$~A\,m$^{-2}$ \\
...
$\phi_s = V_{\text{cell}}$ (swept parameter) \\
...
The mesh and solver settings are summarised in Table~\ref{tab:mesh-solver}. 
Convergence was achieved in one iteration per voltage step, with linear residuals 
on the order of $10^{-15}$--$10^{-16}$ (see solver log in 
Appendix~\ref{app:solver-log}), and the global current balance error was below 
0.01\% at every step.

\begin{table}[h]
\centering
\caption{Mesh and solver parameters used in the COMSOL model.}
\label{tab:mesh-solver}
\begin{tabular}{ll}
\toprule
Parameter & Value \\
\midrule
\multicolumn{2}{c}{\hspace*{-2cm}\textbf{Mesh}} \\
Mesh type & Physics-controlled triangular, ``Finer'' preset \\
Domain size & $360\;\mu\text{m} \times 100\;\mu\text{m}$ \\
Number of triangular elements & 556 \\
Number of vertices & 314 \\
Number of edge elements & 70 \\
Total mesh area & $36{,}000\;\mu\text{m}^2$ \\
Average element quality (skewness) & 0.9148 \\
Minimum element quality & 0.7245 \\
\midrule
\multicolumn{2}{c}{\textbf{Solver and sweep}} \\
Solver type & Default direct linear solver (automatic) \\
Relative tolerance & $1 \times 10^{-4}$ \\
Parametric sweep & $V_{\text{cell}} = 0.2$ to $1.0$ V, step $0.1$ V (9 points) \\
\bottomrule
\end{tabular}
\end{table}

\section{Solver Log}
\label{app:solver-log}

The following log excerpt shows convergence behaviour at each voltage step:
\begin{verbatim}
Parameter V_cell = 0.2.
Iter   SolEst   Damping   Stepsize   LinErr   LinRes
   1     0.81    1.0000      0.81    2.3e-15  2.6e-16
...
Parameter V_cell = 1.0.
   1     0.96    1.0000      0.96    5.0e-15  2.9e-16
\end{verbatim}

\section{Model Validation}

With the experimentally determined conductivity and membrane 
thickness, the effective membrane resistance extracted from 
the slope of the polarisation curve (Figure~\ref{fig:comsol-iv}) 
is 
\[
R = \frac{\Delta V}{\Delta I} = \frac{1.0\ \text{V}}{3.60\ \text{A}} 
\approx 0.278\ \Omega .
\]
The agreement between this value and the bulk resistance 
\(R_b = 0.276\ \Omega\) found from the Nyquist intercept 
(Section~\ref{sec:conductivity}) is better than \(0.7\%\). 
This near perfect agreement between these two parameters that 
have been measured independently via EIS conductivity and 
membrane thickness indicates the validity of both the internal 
consistency and the error propagation into the continuum model, 
which completes the multiscale validation. The continuum model 
is now parameterized with experimental data and has not used 
literature data in its construction.
