...
Two measures were computed from the production run: mean square displacement of the phosphorus atoms-calculated using \texttt{compute msd} taking centre-of-mass correction, time-averaged every 100 steps-and radial distribution function of P atoms with bridging oxygen atoms. A bounded MSD-i.e. One that shows no steady-state growth-reveals that the phosphate group is covalently tethered rather than diffusing, which is characteristic of acid retention. Free diffusion with subsequent leaching of the acid would yield a linearly growing MSD, see \cite{64}.The RDF, g(r) between P atoms and bridging O atoms, using 100 bins and averaged over the full 500~ps production run with \texttt{compute rdf}, was analyzed to monitor the covalent P-O-C linkage integrity:
